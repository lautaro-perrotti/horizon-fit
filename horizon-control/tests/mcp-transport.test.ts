...
import { MCP_TOOL_NAME_PATTERN, listRegisteredTools } from "../src/mcp/index.js";
import {
  buildTestApp,
  closeServer,
  listenHorizon,
...
describe("MCP HTTP transport (JWT + adapters, fixtures)", () => {
  it("client → JWT → /mcp tools/call → catalog_get_product 001-TOP-AZU", async () => {
  ...
  it("ops_health, merchant_get_diagnostics, repo_status, audit_history go through /mcp", async () => {
    const { services, keys } = await buildTestApp();
    const { server, base } = await listenHorizon(services);
    servers.push(server);
  ...
  it("does not skip MCP: tools/list is served by the Streamable HTTP adapter", async () => {
    ...
    expect(names).toContain("catalog_get_product");
    expect(names).toContain("catalog_search_products");
    expect(names).toContain("ops_health");
    expect(names).toContain("repo_status");
    expect(names).not.toContain("catalog.get_product");
    expect(names).not.toContain("ops.health");
    expect(names).not.toContain("repo.status");
    ...
    expect(names.sort()).toEqual([...listRegisteredTools()].sort());
  });

  it("accepts historic dotted names on tools/call without listing them", async () => {
    const { services, keys } = await buildTestApp();
    const { server, base } = await listenHorizon(services);
    servers.push(server);
    const token = await signToken(keys.privateKey, { client: "claude", scopes: CLIENT_SCOPES.claude });
    ...
    expect(call.isError).toBe(false);
    expect(call.tool).toMatchObject({ sku: "001-TOP-AZU" });
  });

  it("catalog_search_products finds SKU 001-TOP-AZU", async () => {
    const { services, keys } = await buildTestApp();
    const { server, base } = await listenHorizon(services);
    servers.push(server);
    const token = await signToken(keys.privateKey, { client: "cursor", scopes: CLIENT_SCOPES.cursor });
    await mcpInitialize(base, token);
    const call = await mcpCallTool(base, token, "catalog_search_products", { sku: "001-TOP-AZU" });
    expect(call.isError).toBe(false);
    const products = (call.tool?.products as Array<{ sku?: string; parent_sku?: string }>) ?? [];
    expect(products.some((product) => product.sku === "001-TOP-AZU" || product.parent_sku === "001-TOP-AZU")).toBe(
      true,
    );
  });
});
